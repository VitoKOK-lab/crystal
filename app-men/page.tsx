...
// Real strung beads sit snug against each other with no cord visible between
// them; pure tangent-circle placement leaves a sliver of visible cord next to
// every bead, so stones/spacers render larger than their tangent-implied
// diameter to close that gap. A flat multiplier alone isn't enough for tiny
// spacers (5mm) sitting next to 10-20mm stones — the multiplier barely grows
// them, so the gap stays visible right where the spacer sits — so spacer/
// stone diameters are floored to a minimum of 9mm for rendering purposes
// only (real mm, used for wrist capacity/price, is untouched) before the
// multiplier is applied. Charms are unaffected — they hang outward from the
// loop rather than sitting inline in the chain.
const BEAD_OVERLAP = 1.2;
